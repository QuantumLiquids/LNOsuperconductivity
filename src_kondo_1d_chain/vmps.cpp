...
//    mpo_gen.AddTerm(Jz, Sz, i + 1, Sz, i + 3);
  ...
  const size_t ref_loc = std::min(ref_elec + 1, N - 1);                      // the paired odd site
  ...
  // Two-site correlation measurements (itinerant electrons, even sites)
  using OpT = Tensor;
  ...
  for (const auto &[label, op1, op2] : two_site_meas_ops) {
    do_job([&]() {
      auto measu_res = MeasureTwoSiteOpGroup(mps, kMpsPath, op1, op2, ref_elec, elec_targets);
  ...
  for (const auto &[label, op1, op2] : loc_two_site_meas_ops) {
    do_job([&]() {
      auto measu_res = MeasureTwoSiteOpGroup(mps, kMpsPath, op1, op2, ref_loc, loc_targets);
      DumpMeasuRes(measu_res, label + file_postfix);
    });
  }
