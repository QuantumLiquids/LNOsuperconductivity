...
% -----------------------------------------------------------------------------
% Index convention health-check (localized-spin dataset)
%
% Expected sources:
...
%
% For THIS script (localized spins), acceptable inputs are:
% - geometry indices
% - raw localized indices (all odd): idx_geom = (idx_raw - 1)/2
%
% If you accidentally load itinerant data (all even raw indices), fail loudly.
% -----------------------------------------------------------------------------
...
elseif all(mod(raw_indices, 2) == 1)
    % Raw localized indices
    ref_site_idx = (ref_site_raw - 1) / 2;
    site_indices = (target_site_idx' - 1) / 2;
elseif all(mod(raw_indices, 2) == 0)
    error(['Loaded an all-even index dataset while reading lszsz/lspsm/lsmsp. ' ...
           'This looks like itinerant-spin data (szsz/spsm/smsp). Fix the file prefix.']);
else
