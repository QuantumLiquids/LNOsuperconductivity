% plot/kondo_1d_chain/plot_localized_spin_corr_1d_chain.m
%
% Purpose
%   Plot equal-time spin correlations S_i · S_j for the 1D Kondo chain on localized electrons
%   for
...
%   lszszJk<JK>U<U>L100D<D>.json
%   lspsmJk<JK>U<U>L100D<D>.json
%   lsmspJk<JK>U<U>L100D<D>.json
%
% Notes
...
for k = 1:nset
    ...
    patt_sz = sprintf('lszsz%s*.json', base_token);
    patt_pm = sprintf('lspsm%s*.json', base_token);
    patt_mp = sprintf('lsmsp%s*.json', base_token);
